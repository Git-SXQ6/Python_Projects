...
class Poster:
    ...
    def create_poster(self):
        ...
        for day, gigs in self.data_handler.gigs.items():
            canvas.create_text(400, y_position, text=day, font=("Arial", 24, "bold"), fill="white", anchor=tk.CENTER)
            y_position += 50
            for i, gig in enumerate(gigs):
                font_size = 20 if i == 0 else 16
                canvas.create_text(400, y_position, text=gig, font=("Arial", font_size), fill="white", anchor=tk.CENTER)
                y_position += 40
            y_position += 30