...
def add_festival_goer():
    ...
    ticket_type = ticket_var.get().strip()

    if not (name and age and distance and accommodation and ticket_type):
        messagebox.showwarning("Missing Information", "Please fill in all fields.")
        return

    if name.isdigit():
        messagebox.showerror("Invalid Name", "Name cannot be only numbers.")
        return

    # Fix: prevent ValueError by validating numeric input before conversion
    try:
        age = int(age)
    except ValueError:
    ...
    if age <= 0:
        messagebox.showerror("Invalid Age", "Age must be greater than 0.")
        return

    # Fix: handle non-numeric distance input safely
    try:
    ...
    if distance < 0:
        messagebox.showerror("Invalid Distance", "Distance cannot be negative.")
        return

    success = data_handler.add_festival_goer(
        name,
        age,
        distance,
        accommodation,
    ...
    if not success:
        messagebox.showwarning("Duplicate Entry", "This festival-goer has already been added.")
        return
...
for ticket_type, color in zip(["Full Access", "1 Day", "2 Day"], ["#B22222", "#8FBC8F", "#DC143C"]):
...
right_frame = tk.Frame(root, padx=10, pady=10, bg="#8FBC8F")
